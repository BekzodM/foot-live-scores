...
export default function HomePage() {
    ...
    useEffect(() => {
        ...
        apiResponse.forEach((item) => {
            ...
            if (leagueId === 2 || leagueId === 3 || leagueId === 39 || leagueId === 61 || leagueId === 78 || leagueId === 135 || leagueId === 140 ) {
                tempFixturesByLeague[leagueId].push(item);
            }
            else if(leagueId === 1 || leagueId === 4 || leagueId === 6 || leagueId === 7 || leagueId === 18 || leagueId === 29 || leagueId === 30 || leagueId === 31 || leagueId === 32 || leagueId === 33 || leagueId === 34 ) {
        ...
        <div className="fixturesContainer">
            ...
            {fixturesByLeague[39]?.length > 0 ? (
                fixturesByLeague[39].map((item, index) => (
                    <div key={index}>
                        <p>{item.teams.home.name} vs {item.teams.away.name}</p>
                        <p className="score">{item.goals.home} {item.fixture.status.short} {item.goals.away}</p>
                    </div>
                ))
            ) : (
